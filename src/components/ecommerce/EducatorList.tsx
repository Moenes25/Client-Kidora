...
export default function EducatorList() {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
      ...
      {/* Section pour les éducateurs non présents */}
      {/* {otherEducators.length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {otherEducators.length} non présents aujourd'hui
              </p>
            </div>
          ...
          <div className="space-y-3">
            {visibleOtherEducators.map((educator) => (
              <div 
                key={educator.id} 
                className="flex items-center justify-between p-3 rounded-lg transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-700/50"
              >
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white dark:border-gray-800">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${getStatusColor(educator.status)}`}>
                        {getStatusText(educator.status)}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500 dark:text-gray-400 truncate">
                      {educator.role}
                ...
                <div className="flex flex-col items-end">
                  <span className="text-sm font-medium px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
                    {educator.assignedClass}
                  </span>
                </div>
              </div>
            ...
            {otherEducators.length > defaultVisibleCount && !showAllTeam && (
              <div className="flex -space-x-2 pt-2">
                {otherEducators.slice(defaultVisibleCount).map((educator) => (
                  <div 
                    key={educator.id} 
                    className="relative w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 overflow-hidden group"
                    title={`${educator.name} - ${getStatusText(educator.status)}`}
                  >
                    {imageErrors[educator.id] ? (
                      <div className={`w-full h-full ${
                        educator.status === 'absent' 
                ...
                {otherEducators.length - defaultVisibleCount > 4 && (
