...
export default function ChildrenToday({ enfants: enfantsExternes }: ChildrenTodayProps) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
        ...
        <div className="flex space-x-2">
          <button 
            ...
            className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === 'all' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 font-medium' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Tous ({enfants.length})
          </button>
          <button 
            onClick={() => setFilter('present')}
            className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === 'present' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400 font-medium' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Présents ({countPresent})
          </button>
          <button 
            onClick={() => setFilter('absent')}
            className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === 'absent' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 font-medium' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
          >
            Absents ({countAbsent})
          </button>
        </div>
      ...
      {/* Grille des enfants (3-4 par ligne) */}
      <div className="overflow-y-auto pr-1">
        {enfantsFiltres.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
            <UserIcon className="size-10 mx-auto mb-2 text-gray-400" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {enfantsFiltres.map(enfant => (
              <div 
                key={enfant.id} 
                className="flex flex-col items-center p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors group"
              >
                {/* Photo et statut */}
                <div className="relative mb-3">
                  <div className={`w-16 h-16 rounded-full overflow-hidden border-2 ${enfant.present ? 'border-green-200 dark:border-green-800' : 'border-red-200 dark:border-red-800'}`}>
                    {enfant.photo ? (
                      <img 
                        src={enfant.photo} 
                        alt={enfant.nom}
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            parent.innerHTML = `
                              <div class="w-full h-full flex items-center justify-center ${enfant.present ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}">
                                <span class="font-medium ${enfant.present ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}">
                      ...
                      <div className={`w-full h-full flex items-center justify-center ${enfant.present ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                        <span className={`font-medium ${enfant.present ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  ...
                  {/* Badge d'état */}
                  <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-white dark:border-gray-900 flex items-center justify-center ${enfant.present ? 'bg-green-500' : 'bg-red-500'}`}>
                    {enfant.present ? (
                ...
                <div className="text-center mb-2">
                  <div className="font-medium text-gray-900 dark:text-white text-sm truncate w-full">
                    {enfant.nom.split(' ')[0]}
                  </div>
                ...
                <div className="text-center">
                  <div className={`text-xs px-2 py-1 rounded-full font-medium ${enfant.present ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                  ...
                  <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {enfant.present ? (
                      <div className="flex items-center justify-center">
                        <CheckCircleIcon className="size-2 mr-1 text-green-500" />
                  ...
                  {enfant.classe && (
                    <div className={`mt-1 text-xs font-medium px-2 py-0.5 rounded ${
                      enfant.classe.includes("3-4") ? "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400" :
                      enfant.classe.includes("4-5") ? "bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400" :
                      enfant.classe.includes("6-7") ? "bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400" :
                      enfant.classe.includes("8-9") ? "bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400" :
                      enfant.classe.includes("10-11") ? "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400" :
      ...
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg flex items-center">
            <div className="mr-3">
              <CloseIcon className="size-5 text-red-600 dark:text-red-400" />
            </div>
            <div>
        ...
        {/* Bouton d'action */}
        <div className="mt-4">
          <button className="w-full py-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors text-sm font-medium">
